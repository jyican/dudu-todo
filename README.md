# dudu tools (floating-todo)

一个悬浮在桌面、始终置顶的待办清单 + 小工具箱,数据全部存本地,**每 3 小时**对未完成项发一次系统通知。基于 Tauri 2,跨平台(macOS / Windows / Linux)。
...
- ➕ 随手添加:一句话 + 可选截止日期
- ✏️ 标题可随时点 ✎ 内联修改
- 🌲 每个待办可展开添加**子待办**,子待办的标题与内容都可编辑
- ✅ 待做 / 已完成两个分区,点圆圈切换,悬停出现删除
- 🧰 工具箱(标题栏 🧰):
  - **转码**:URL / Base64 编码与解码
  - **JSON**:格式化与压缩
  - **时间戳**:Unix 秒 / 毫秒 ↔ 本地时间 / UTC / ISO 互转
...
- 首次运行 macOS 会弹通知授权,需在「系统设置 → 通知」允许「dudu tools」,否则收不到提醒。
